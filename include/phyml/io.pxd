...
cdef extern from "io.h" nogil:
    ...
    void Print_Fp_Out(FILE *fp_out,time_t t_beg,time_t t_end,t_tree *tree,t_option *io,int n_data_set,int num_tree, int add_citation, int precision)
    void Print_Fp_Out_Lines(FILE *fp_out,time_t t_beg,time_t t_end,t_tree *tree,t_option *io,int n_data_set)
    void Print_Freq(t_tree *tree)
    void Print_Settings(t_option* io)
    void Print_Banner(FILE *fp)
    void Print_Banner_Small(FILE *fp)
    void Print_Data_Set_Number(t_option *io,FILE *fp)
    ...
    t_tree* Read_User_Tree(t_calign* cdata, t_mod* mod, t_option* io)
    ...
    t_option* Get_Input(int argc,char **argv)
