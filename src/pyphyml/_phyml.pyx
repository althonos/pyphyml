...
cdef t_option* Get_Input(int argc, char** argv):
...
def main(filename):
    ...
    cdef char** argv = <char**> calloc(sizeof(char*), 3)
    argv[0] = strdup(b"phyml")
    argv[1] = strdup(b"-i")
    argv[2] = strdup(os.fsencode(filename))
    ...
    cdef void _post_process(self) except *:
        ...
        for i in range(self._data[0].len):
            for j in range(self._n_otu):
                ...
                if (self._datatype == t_type.NT) and (self._data[j].state[i] == ord('N')):
                    self._data[j].state[i] = ord('X')
                if self._data[j].state[i] == ord('U'):
