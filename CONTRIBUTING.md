...
Python objects should be typed; since it is not natively supported by Cython,
...
Python 3.5) or in variable assignments (supported from Python 3.6 onward). 
Cython allows you to use [f-strings](https://www.python.org/dev/peps/pep-0498/)
when compiling the code for any version.
...
wrapped code of PhyML which is exposed as a ``git`` submodule:

```console
$ git clone --recursive https://github.com/althonos/pyphyml
```
